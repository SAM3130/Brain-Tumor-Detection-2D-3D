            ...
            if use_list:
                ...
                if type(dt) in list(six.integer_types) + [bool]:
                    ...
                    try:
                        ...
                        raise TypeError(
                            "Unsupported type to batch: {}".format(type(dt))
                        )
                try:
...
def sampler3d(volume_list, label, weight):
    ...
    center_point = get_random_roi_sampling_center(
        volume_shape, sub_label_shape, "full", None
    )
    sub_label = extract_roi_from_volume(
        label, center_point, sub_label_shape, fill="zero"
    )
    sub_data = []
    flip = False
    for moda in range(len(volume_list)):
        sub_data_moda = extract_roi_from_volume(
            volume_list[moda], center_point, sub_data_shape
        )
        if flip:
    ...
    sub_weight = extract_roi_from_volume(
        weight, center_point, sub_label_shape, fill="zero"
    )
    if flip:
